...
if __name__ == "__main__":
    ...
    cells = pd.read_csv(sys.argv[2])
    presc = pd.read_csv(sys.argv[3])
    procsc = pd.read_csv(sys.argv[4])
    outpath = sys.argv[5]
    ...
    # reformat and merge intensity scores
    presc['name'] = 'prepared'
    procsc['name'] = 'processed'
    
    presc = reformatScores(presc)
    procsc = reformatScores(procsc)

    scores = pd.merge(presc, 
                      procsc, 
                      how='left',
                      left_on=['t', 'label'],
                      right_on=['t', 'label'])
